...
1. 各入力テキストに `unicodedata.normalize("NFKC", t)` を適用して全角→半角に変換
2. 空でない入力テキストを `","` でマージ
3. マージ後テキストが `{` で始まる場合（JSON形式）は正規表現を適用せずそのまま返す
4. 正規表現パターンにマッチした箇所を `""` で削除
5. 無効な正規表現パターンが入力された場合は置換しない
6. コメント部分や余分なスペース、カンマを削除して整形
7. `deduplicate` が ON の場合、カンマ分割 → 各タグをトリム → 重複除去（順序保持）→ 再結合
8. マージ後と置換後のテキストをノード上に表示
9. 「入力数を更新」ボタンを押すと `input_count` の値に合わせて入力ソケットが追加・削除される
...
### Load Image (Out Path) (`custom_nodes/regex_replace_node/`)

ComfyUI 標準の `LoadImage` を継承し、画像パスを追加出力するノード。

**ファイル構成:**
- `custom_nodes/regex_replace_node/nodes/load_image_out_path.py` — `LoadImageOutPath` クラス本体

#### 出力

| 名前 | 種類 | 説明 |
|------|------|------|
| image | IMAGE | 読み込んだ画像テンソル |
| mask | MASK | マスク |
| path | STRING | 画像ファイルの絶対パス（`/` 区切り、末尾に `\n\n` 付き） |

#### 動作仕様

- `folder_paths.get_annotated_filepath(image)` でファイルパスを取得
- パス区切り文字を `\\` → `/` に統一
- パスの末尾に `\n\n` を付加して返す（テキスト連結時の区切りとして利用）
